...
import { ShoppingCart, User, Phone, MapPin, FileText, ArrowRight, Store, Truck } from "lucide-react";
...
.insert(orderItems);
      ...
      alert(`✅ تم إرسال الطلب بنجاح\n\nعدد الطلبات: ${groupedByShop.length}\nالإجمالي: ${total.toFixed(2)} ج.م + مصاريف الشحن\n\nسيتم التواصل معك قريباً`);
      navigate("/");
    } catch (error) {
      console.error(error);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          <button
            ...
            className="bg-[#1E1E1E] border border-[#333] p-3 rounded-xl hover:bg-[#2a2a2a] transition"
          >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl p-4 bg-[#121212] text-white border border-[#333] focus:border-[#D4AF37] focus:outline-none transition"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl p-4 bg-[#121212] text-white border border-[#333] focus:border-[#D4AF37] focus:outline-none"
                    required
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-2 flex items-center gap-2">
                    <MapPin size={16} />
                    العنوان بالتفصيل أو لينك الموقع على الخريطة *
                  </label>
                  <textarea
                    name="address"
                    value={form.address}
                    placeholder="مثال: شارع التحرير، عمارة 5، الدور 3، شقة 10&#10;أو: https://maps.app.goo.gl/..."
                    rows="3"
                    onChange={handleChange}
                    className="w-full rounded-xl p-4 bg-[#121212] text-white border border-[#333] focus:border-[#D4AF37] focus:outline-none resize-none"
                    required
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    اكتب العنوان كامل أو الصق لينك Google Maps
                  </p>
                </div>
                ...
                <button
                  type="submit"
                  disabled={submitting || shopsBelowMin.length > 0}
                  className="w-full py-4 rounded-xl bg-[#D4AF37] text-black font-bold text-lg hover:bg-[#D4AF37]/90 transition-transform hover:scale-105 disabled:opacity-50 disabled:cursor-wait disabled:hover:scale-100"
                >
                  {submitting? 'جاري الإرسال...' : 'تأكيد الطلب'}
                </button>

                {shopsBelowMin.length > 0 && (
                  <p className="text-yellow-400 text-sm text-center">
                    ⚠️ بعض المحلات لم تصل للحد الأدنى للطلب
                  </p>
                )}
              </form>
            </div>
            ...
            <div className="bg-[#1E1E1E] border border-[#333] rounded-2xl p-6 sticky top-6">
                  ...
                  return (
                    <div key={shop.shopId} className="bg-[#121212] border border-[#333] rounded-xl p-4">
                      <div className="flex items-center gap-2 mb-3 pb-2 border-b border-[#333]">
                        ...
                        <p className="font-bold text-[#D4AF37] text-sm">{shop.shopName}</p>
                      </div>
                        ...
                        {shop.items.map((item) => (
                          <div key={item.product_id} className="flex items-center gap-2 text-sm">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-white truncate text-xs">{item.name}</p>
                      ...
                      <div className="mt-3 pt-3 border-t border-[#333] space-y-1 text-xs">
                        <div className="flex justify-between text-gray-400">
                          ...
                          <span className="text-white font-bold">{shop.subtotal.toFixed(2)} ج.م</span>
                        </div>
                        <div className="flex justify-between text-gray-400">
                        ...
                        {parseFloat(shop.shopData?.min_order || 0) > 0 && shop.subtotal < shop.shopData.min_order && (
                          <div className="text-yellow-400 text-xs">
                            ⚠️ الحد الأدنى: {parseFloat(shop.shopData.min_order).toFixed(2)} ج.م
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <div className="border-t border-[#333] pt-4 space-y-3">
                <div className="flex justify-between text-gray-400">
                  ...
                  <span className="font-bold text-white">{subtotal.toFixed(2)} ج.م</span>
                </div>
                <div className="flex justify-between text-gray-400">
                  <span>رسوم التوصيل:</span>
                  {totalDeliveryFee > 0? (
                    <span className="font-bold text-white">{totalDeliveryFee.toFixed(2)} ج.م</span>
                  ) : (
                    <span className="text-[#D4AF37] font-bold text-xs">
                      تابع مع شركة الشحن
                    </span>
                ...
                <div className="border-t border-[#333] pt-3">
                  ...
                  {totalDeliveryFee === 0 && (
                    <p className="text-xs text-[#D4AF37] mt-1 text-left">
                      + رسوم التوصيل (تابع مع شركة الشحن)
                    </p>
                  )}
                </div>
              </div>
